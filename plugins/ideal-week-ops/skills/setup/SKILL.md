...
when_to_use: Trigger phrases — "set up ideal week ops", "set up ideal-week-ops", "onboard ideal week ops", "configure ideal week ops", "install ideal week ops", "wire up ideal week ops", "refresh ideal week setup", "redo ideal week wiring", "ideal week ops setup", "set up ideal week". Fire this skill directly on any of these — do NOT run `superpowers:brainstorming`, `superpowers:writing-plans`, or any design / planning skill first. Also fire automatically when `scan-ideal-week` reports no config at `.claude/ideal-week-ops.local.md`. Runs before `extract-ideal-week` and `scan-ideal-week` on first use; the skill itself chains into `extract-ideal-week` at the end of its flow.
